/**
 * REST controller for OMP engine config management.
 *
...
import type { JsonValue } from './omp-schema/serde_json/JsonValue';
import {
...
@ApiTags('omp-engine-config')
@ApiBearerAuth()
@ApiUnauthorizedResponse({ type: ApiErrorResponseDto })
@Controller('omp/engine-config')
export class OmpEngineConfigController {
  private readonly logger = new Logger(OmpEngineConfigController.name);
  ...
  private async readConfigFromAppServer(): Promise<v2.ConfigReadResponse> {
    return this.ompService.request<v2.ConfigReadResponse>('config/read', {
      includeLayers: true,
  ...
  private isConfigValidationError(error: unknown): error is OmpRpcError {
    if (!(error instanceof OmpRpcError)) return false;
