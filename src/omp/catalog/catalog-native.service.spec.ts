...
import {
  ...
  OmpStartupError,
} from '../omp-process-manager.service';
...
describe('pinned catalog parser and lifecycle', () => {
  ...
  let manager: OmpProcessManager;
  beforeEach(() => {
    fixture = catalogFixture();
    native = new CatalogNativeService(fixture.paths);
    manager = new OmpProcessManager(fixture.storage);
  });
  afterEach(async () => {
  ...
  it('distinguishes catalog parse/read errors from credentials, inference and timeouts', () => {
    for (const message of [
      ...
      expect(
        isCatalogStartupFailure(new OmpStartupError('start', message)),
      ).toBe(true);
    for (const message of [
      'expired credentials',
