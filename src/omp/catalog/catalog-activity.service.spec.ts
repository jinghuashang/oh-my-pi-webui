...
describe('CatalogActivityService', () => {
  ...
  let manager: {
    getClient: Mock<
      () => { request: typeof request; acceptedWork: OmpAcceptedWork } | null
    >;
  ...
  beforeEach(() => {
    ...
    manager = {
      getClient: vi
        .fn()
        .mockReturnValue({ request, acceptedWork: new OmpAcceptedWork() }),
      getGeneration: vi.fn().mockReturnValue(3),
    };
    service = new CatalogActivityService(
      manager as unknown as OmpProcessManager,
      admission,
    );
  });
