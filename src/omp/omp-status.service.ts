/**
 * Aggregates OMP engine readiness, account, config, provider, and model status.
 */
...
import type { v2 } from './omp-schema';
...
export interface OmpAppServerStatus {
  ok: boolean;
  connected: boolean;
  initialized: boolean;
  error?: CodexStatusError;
...
export interface OmpProviderStatus {
  ok: boolean;
  id: string | null;
  name: string | null;
...
@Injectable()
export class OmpStatusService {
  private readonly logger = new Logger(OmpStatusService.name);
  private cache: CacheEntry | null = null;
  private inFlight: Promise<CodexStatusResponse> | null = null;

  constructor(
    private readonly processManager: OmpProcessManager,
   ...
   */
  async getProviderStatus(): Promise<OmpProviderStatus> {
  ...
  private async buildStatus(): Promise<CodexStatusResponse> {
    ...
    if (!client || !initResult) {
      return this.buildUnavailableStatus({
        ...
        message: client
          ? 'OMP engine is connected but not initialized'
  ...
  private async probe<T>(
    ...
    try {
      const data = await this.ompService.request<T>(method, params);
      return { ok: true, data };
    } catch (err) {
  ...
  private hasNonEnvCredential(providerId: string, config?: v2.Config): boolean {
    if (providerId.toLowerCase() === 'omp') return true;
    const providerConfig = this.lookupProviderConfig(providerId, config);
  ...
  private buildUnavailableStatus(args: {
    ...
    const skipped: CodexStatusError = {
      code: 'SKIPPED',
      message: 'Skipped because OMP engine is unavailable',
    };
