...
import { OmpEngineConfigController } from './omp-config.controller';
import { OmpService } from './omp-engine.service';
import { OmpStatusService } from './omp-status.service';
import { isCodexConfigEditableKey } from './dto/omp-config.dto';
import type { v2 } from './omp-schema';

describe('OmpEngineConfigController', () => {
  let moduleRef: TestingModule;
  let controller: OmpEngineConfigController;
  ...
  beforeEach(async () => {
    ...
    moduleRef = await Test.createTestingModule({
      controllers: [OmpEngineConfigController],
      providers: [
  ...
  it('preserves the original RPC error for multi-edit validation failures', async () => {
    const rpcError = new OmpRpcError(
      {
        code: -32600,
        message: 'invalid approvals reviewer',
        data: { config_write_error_code: 'configValidationError' },
