/** REST controller for OMP feedback uploads. */
...
} from './dto/omp-feedback.dto';

type FeedbackBodyRecord = Record<string, unknown>;

@ApiTags('omp-feedback')
@ApiBearerAuth()
@ApiUnauthorizedResponse({ type: ApiErrorResponseDto })
@Controller('omp/feedback')
export class OmpFeedbackController {
  constructor(private readonly ompService: OmpService) {}

  /** Uploads a feedback report through OMP engine. */
  @Post()
  @ApiOperation({ summary: 'Upload a OMP feedback report' })
  ...
  uploadFeedback(
    ...
    return this.ompService.request<v2.FeedbackUploadResponse>(
      'feedback/upload',
