...
describe('OmpJsonRpcClient', () => {
  let proc: ChildProcess;
  let client: OmpJsonRpcClient;

  beforeEach(() => {
    proc = createMockProcess();
    client = new OmpJsonRpcClient(proc);
  });
  ...
  it('should reject pending requests on timeout', async () => {
    const shortClient = new OmpJsonRpcClient(proc, 50);
...
describe('OmpJsonRpcClient accepted work', () => {
  let proc: ChildProcess;
  let client: OmpJsonRpcClient;
  ...
  beforeEach(() => {
    proc = createMockProcess();
    client = new OmpJsonRpcClient(proc, 20);
  });
  afterEach(() => {
    client.destroy();
