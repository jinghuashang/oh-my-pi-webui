...
import type { InitializeResponse, v2 } from './omp-schema';
...
describe('OmpStatusService', () => {
  let service: OmpStatusService;
  ...
  beforeEach(async () => {
    moduleRef = await Test.createTestingModule({
      providers: [
        OmpStatusService,
        { provide: OmpProcessManager, useValue: mockProcessManager },
        { provide: OmpService, useValue: mockCodex },
        { provide: ConfigService, useValue: mockConfig },
      ],
    }).compile();

    service = moduleRef.get(OmpStatusService);
