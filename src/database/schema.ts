/** Drizzle table declarations for WebUI persistence. */
import {
  index,
  integer,
  primaryKey,
 ...
 * WebUI account.
 *
 * One row per human login. The password is stored as a scrypt derivation with
 * its own random salt; nothing reversible is ever persisted. The table exists
 * from the first boot but stays empty until the initialization flow runs, which
 * is what makes "uninitialized" a database fact rather than a config flag.
 */
export const users = sqliteTable(
  'users',
  {
    ...
    /** Constant column: the unique index below makes a second account impossible. */
  ...
  (table) => [
    ...
    uniqueIndex('idx_users_singleton').on(table.singleton),
  ],
);
...
/**
 * Server-owned secrets that must survive restarts (JWT signing secret).
 *
 * Kept out of `settings` because nothing here is user-editable; the row is
 * written once on first boot and never surfaced through the settings API.
 */
...
export type ServerSecretRow = typeof serverSecrets.$inferSelect;
