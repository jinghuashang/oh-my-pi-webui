...
export class LogsService {
  ...
  async exportDiagnostics(): Promise<LogsExportResponse> {
    const [{ data: logs }, runtimeStatus, engineVersion, webuiVersion] =
      await Promise.all([
  ...
  /** Version banner the bridge reports, which carries the omp version behind it. */
  private async getEngineVersion(): Promise<string> {
    const bin = this.config.get<string>('BRIDGE_BIN') ?? resolve('bridge/dist/index.js');
    const isJs = bin.endsWith('.js') || bin.endsWith('.cjs');
    const exec = isJs ? process.execPath : bin;
    const args = isJs ? [bin, '--version'] : ['--version'];
    try {
