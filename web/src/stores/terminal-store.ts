...
const STORAGE_KEY = 'omp-webui.terminal-tabs.v1';
/** Key used before the rename; migrated on first read. */
const LEGACY_STORAGE_KEY = 'codex-webui.terminal-tabs.v1';

// The key was renamed; adopt the old value once so open tabs survive the change.
try {
  if (!sessionStorage.getItem(STORAGE_KEY)) {
    const previous = sessionStorage.getItem(LEGACY_STORAGE_KEY);
    if (previous) {
      sessionStorage.setItem(STORAGE_KEY, previous);
