...
interface LayoutState {
  ...
  /** Whether the session side panel (git, goal, progress) is shown. */
  sessionSidePanelOpen: boolean;
...
export const useLayoutStore = create<LayoutState>()(
  persist(
    (set, get) => ({
      ...
      toggleSessionSidePanel: () =>
        set((state) => ({ sessionSidePanelOpen: !state.sessionSidePanelOpen })),
    ...
    {
      name: 'omp.webui.layout',
      partialize: (state) => ({
        desktopSidebarCollapsed: state.desktopSidebarCollapsed,
        collapsedGroupKeys: state.collapsedGroupKeys,
        sessionSidePanelOpen: state.sessionSidePanelOpen,
      }),
    },
  ),
);
