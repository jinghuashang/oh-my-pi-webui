...
import { formatAbsoluteAge, formatRelativeAge } from '@/lib/relative-time';
...
export function ThreadRow({
  ...
  const age = formatRelativeAge(thread.updatedAt, i18n.language);
  const ageTitle = formatAbsoluteAge(thread.updatedAt, i18n.language);
  ...
  return (
    <div className="group flex items-center gap-0.5 pl-3">
      ...
      >
        ...
        {age && (
          <span
            className="ml-auto shrink-0 pl-1.5 text-[10px] tabular-nums text-muted-foreground/60"
        ...
        {pendingApprovalCount > 1 && (
          <span
            className={cn(
              'shrink-0 rounded-full bg-yellow-500/15 px-1.5 py-0.5 text-[10px] font-medium leading-none text-yellow-600 dark:text-yellow-400',
              !age && 'ml-auto',
            )}
          >
            {badgeText}
