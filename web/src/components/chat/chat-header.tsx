...
import { Check, Edit3, EllipsisVertical, FolderGit2, GitBranch, Globe, Menu, Moon, Network, PanelLeftOpen, PanelRight, Settings, Sun, X } from 'lucide-react';
...
import { gitStatus } from '@/generated/api';
  ...
  return (
    <Popover open={open} onOpenChange={setOpen}>
      ...
      <PopoverContent align="end" side="bottom" className="w-48 p-1">
        {onToggleSidePanel && (
          <button type="button" onClick={() => act(onToggleSidePanel)} className="flex w-full items-center gap-2 rounded-md px-3 py-2 text-sm hover:bg-accent">
            <PanelRight className="h-4 w-4" /> {t('Session panel')}
          </button>
        )}
...
export function ChatHeader({ dark, onToggleDark }: Props) {
  ...
  const toggleSessionSidePanel = useLayoutStore((s) => s.toggleSessionSidePanel);

  const workspaceName = useMemo(() => {
    if (!threadCwd) return null;
    const parts = threadCwd.split(/[\\/]/).filter(Boolean);
    return parts[parts.length - 1] ?? null;
  }, [threadCwd]);

  // The branch is repository state, so it comes from git rather than the
  // conversation; a missing repo simply leaves the pill out.
  const branchQuery = useQuery({
    queryKey: ['gitBranch', threadCwd],
    enabled: Boolean(threadCwd),
    staleTime: 20_000,
    queryFn: async () => {
      const { data } = await gitStatus({ query: { cwd: threadCwd! }, throwOnError: true });
      return data;
    },
  });
  const branch = branchQuery.data?.isRepo ? branchQuery.data.branch : null;
      ...
      <header className="glass-bar sticky top-0 z-10 flex items-center gap-2 px-3 py-2.5 sm:gap-3 sm:px-4 sm:py-3 lg:px-6">
        ...
        <div className="min-w-0 flex-1">
          {threadId ? (
            <div className="flex min-w-0 items-center gap-2">
              {editing ? (
                ...
                <>
                  ...
                  {workspaceName && (
                    <span
                      className="hidden min-w-0 items-center gap-1 truncate text-xs text-muted-foreground md:inline-flex"
                      title={threadCwd ?? undefined}
                    >
                  ...
                  {branch && (
                    <Badge
                      variant="outline"
                      className="hidden shrink-0 gap-1 font-mono text-[10px] md:inline-flex"
                    >
            ...
            <h1 className="text-lg font-semibold tracking-tight">Oh My Pi WebUI</h1>
          )}
        </div>
        ...
        {isDesktop ? (
          <>
            <Tooltip>
              <TooltipTrigger asChild>
                <Button size="icon" variant="ghost" className="h-8 w-8" onClick={toggleSessionSidePanel}>
                  <PanelRight className="h-4 w-4" />
                </Button>
              </TooltipTrigger>
              <TooltipContent>{t('Session panel')}</TooltipContent>
            </Tooltip>
            <Tooltip>
              <TooltipTrigger asChild>
          ...
          <MobileOverflowMenu
            onToggleSidePanel={toggleSessionSidePanel}
