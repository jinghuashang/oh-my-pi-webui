...
export function useChatAttachments({
  ...
  useEffect(() => {
    ...
    window.addEventListener('omp-webui:attach-file', handler);
    return () => window.removeEventListener('omp-webui:attach-file', handler);
  ...
  const uploadFile = useCallback(async (file: File): Promise<{ path: string } | null> => {
    ...
    if (!resp.ok) {
      if (resp.status === 401) {
        clearApiToken();
        window.dispatchEvent(new Event('omp-webui:auth-expired'));
      }
