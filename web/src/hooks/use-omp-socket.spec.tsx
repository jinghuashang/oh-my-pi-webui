...
import { useOmpSocket } from './use-omp-socket';
...
function mount() {
  ...
  return renderHook(() => useOmpSocket(), {
    wrapper: ({ children }) => (
...
it('ingests both late activity events into the initiating turn without reopening it or changing another active turn', async () => {
  ...
  const send = (method: string) => act(() => transport.listeners.get('omp.notification')?.({ method, params }));
